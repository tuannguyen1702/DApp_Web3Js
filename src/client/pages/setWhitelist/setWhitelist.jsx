...
import { SetWhitelistModel, SingleWhitelistModel, AddWhitelistModel } from "./setWhitelistModel"
...
class SetWhitelist extends Component {
    ...
    initFormModel() {
        var self = this
        self.form = new FormModel({ ...SetWhitelistModel }, { name: 'SetWhitelistModel' })
        self.singleWhitelistForm = new FormModel({ ...SingleWhitelistModel }, { name: 'SingleWhitelistModel' })
        self.addWhitelistModel = new FormModel({ ...AddWhitelistModel }, { name: 'AddWhitelistModel' })
        ...
        self.form.onSuccess = function (form) {
            ...
            myContract.methods.addAddressesToWhitelist(recipientArr).send({
    ...
    initData() {
        ...
        if (investmentAddress != undefined) {
            self.singleWhitelistForm.$("address").value = investmentAddress
            self.setState({ recipientArr: [investmentAddress] })

        } else {
            NetworkService.getWhitelist().subscribe(
                function onNext(response) {
                    console.log(response)
                    if (response) {
                        var recipientArr = []
                        ...
                        //recipientArr.push(["0xcB1a15656af3DA92e186f4C5abee4c177163fb02"])

                        self.addWhitelistModel.$("addressList").value = recipientArr

                        self.setState({ dataTable: response, recipientArr: recipientArr })
                    }
    ...
    componentDidMount() {
        ...
        self.addWhitelistModel.$("addressList").observe({
            key: 'value',
            call: (data) => {
    ...
    estimateGas() {
        ...
        myContract.methods.addAddressesToWhitelist(recipientArr).estimateGas({
            ...
            <div>
                ...
                <div>
                    <div className="main-title-container">
                        <h2 className="main-title"><FormattedMessage id="Set Whitelist" /></h2>
                    </div>
                    <Form>
                        {/* <Row>
                        ...
                        {investmentAddress != undefined ? <Row>
                            <Col xs="12" sm="6">
                                <TextField disabled field={self.singleWhitelistForm.$("address")} />
                            </Col>
                        </Row> : <Row>
                                ...
                                <Col xs="12" sm="6">
                                    {_.isEmpty(recipientArr) ? <div className="form-group"><label>Address List</label><div className="label-box-container"><span className="label-box form-control">No Address</span></div></div> :
                                        <MultiTextField field={self.addWhitelistModel.$("addressList")} />}
                                    <TextField field={self.addWhitelistModel.$("address")}
                                        onKeyPress={(e) => {
